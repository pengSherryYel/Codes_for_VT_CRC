...
for i in "${variable[@]}"; do
    ...
    cat $merged_fasta_gt1k >>$merged_fasta_all
...
sh ~/script/module_cluster/checkv_cluster_ctg/cluster_ctg_balstn_ani.sh \$before_fasta $i $nr_gpd_ca_opt/$i 30
" >>$script

done

###################################################################
## remove redundant using fastANI for all the cross assemble contig
## because mash take very long time
###################################################################
mash_opt="$opt_dir/mash/all"
mkdir $mash_opt
sh ~/script/module_cluster/checkv_cluster_ctg/cluster_ctg_balstn_ani.sh $merged_fasta_all $mash_opt 30
