...
##sh /ictstr01/home/viro/xue.peng/workplace_2023/CRC_community_vt/src/run_mmseqs_taxonomy.sh \$merged_CA_viral_check_blast_fna $i $mmseqs2_tax_opt/$i
...
echo "
### use ANI 95%|98% remove redundant (script is from checkV code hub)
#mkdir -p $nr_gpd_ca_opt/$i
#before_fasta=\"$nr_gpd_ca_opt/$i/${i}_crossAss_GPD_before_cluster.fasta\"
#cat $gpdSeqF $mash_opt/mashani_rep_id95.0.rename.fasta >\$before_fasta
#sh ~/script/module_cluster/checkv_cluster_ctg/cluster_ctg_balstn_ani_modify.sh \$before_fasta $i $nr_gpd_ca_opt/$i 30 98 85
" >>$script
...
#. ~/script/bt2.sh
### command:
### btbuild cross_assemble_abundance/nr_gpd_crossAss/all/rep.fasta cross_assemble_abundance/nr_gpd_crossAss/all/bt2DB/gpd_CA.rep
...
### because mash take very long time
####################################################################
nr_gpd_ca_all_opt_95="$nr_gpd_ca_opt/all/ani95"
mkdir -p $nr_gpd_ca_all_opt_95
###please make sure above script is done and the renamed file is exists!!!!!
#cat ./cross_assemble_abundance/mash/*/mashani_rep_id95.0.rename.fasta $gpdSeqF >$merged_fasta_all
sh ~/script/module_cluster/checkv_cluster_ctg/cluster_ctg_balstn_ani_modify.sh $merged_fasta_all all_ani_95 $nr_gpd_ca_all_opt_95 30 95 85
gpd_CA_rep_fasta="$nr_gpd_ca_all_opt_95/rep.fasta"
